...
setwd('~/Desktop/R_bocas18_fishes/permanova')
library(vegan)
...
```{r}
data <- read.csv('data/sampling_spots_more.csv',sep='\t') %>% select(-Latittude,-Longitude)
```
...
data <- merge(data,data.scores,by='spot')
```
...
ggplot(data,aes(x=NMDS1,y=NMDS2,group=group)) + 
  ...
  theme_ipsum(grid=F,base_family = "Arial",
...
gO <- data[match(labels(dM),data$spot),]$gr
...
ggplot(data.frame(group=betaM$group,distance=betaM$distances),
  ...
  geom_boxplot()+ggtitle('beta dispersion')+
  scale_fill_ipsum()+
  theme_ipsum(grid=F,base_family = "Arial",
              plot_margin = unit(c(5,5,5,5),'pt'),
              axis_title_face = 'bold')
...
```{r,fig.align='center',out.width = "80%",fig.width=9,fig.height=4}
# choose Color
statC <- ipsum_pal()(4)[4]
# export the summary statistics from the PERMANOVA run
permOUT <- permustats(permM)
# transform into da data.frame
permDF <- permOUT$permutations %>% as.data.frame();names(permDF) <- 'Permutations'

# plotting
ggplot(permDF,aes(x=Permutations))+
  # add the distribution frm the permutations
  geom_density(fill=statC)+
  geom_point(aes(y=0),shape=21,fill=darken(statC,2.5))+
  # add our realized value
  geom_vline(xintercept = permOUT$statistic,col=darken(statC,2.5))+
  # expand x scale 
